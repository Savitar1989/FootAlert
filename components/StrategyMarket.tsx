...
import { Download, Star, Search, ShieldCheck, DollarSign, Lock } from 'lucide-react';
...
export const StrategyMarket: React.FC<StrategyMarketProps> = ({ onImport }) => {
  ...
  const strategies = MOCK_MARKET_STRATEGIES.filter(s => 
      ...
      <div className="grid md:grid-cols-2 gap-5">
        {strategies.map(strat => (
          <div 
            ...
            className="glass-card rounded-2xl p-6 hover:border-indigo-500/30 transition-all group shadow-lg hover:shadow-indigo-500/10 cursor-pointer"
          >
             ...
             <div className="grid grid-cols-3 gap-3 mb-5 bg-slate-950/40 p-3 rounded-xl border border-white/5">
                ...
                <div className="text-center">
                   ...
                   <div className={`text-sm font-bold ${strat.roi >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{strat.roi}%</div>
                </div>
             </div>
